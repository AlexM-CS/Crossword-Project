...
let autocheck = false // Toggle autocheck
...
function isValidTile(row, col){
    // console.log("before",row,col) // DEBUG
     if (inBounds(row, col)) {
         // console.log("after" , row,col) // DEBUG
...
function makeHintDict(jsonData, hints) {
    ...
    // Iterates through hints and finds when hint and the jsonData hints are the same
    // and creates key
...
function handleDirectionSwap(row,col){
    ...
    // If current tile has black tiles to the sides of it sets direction down
...
function handleBlackTileJump(userChar, row, col) {
    ...
    // Iterates until a non-black tile is found
    while (true) {
        jumpIndx++;
...
function handleTextInput(event) {
    ...
    // Sets char to an arrow key to handle movement
    if (direction ){
        userChar = "ArrowRight"
    } else {
        userChar = "ArrowDown"
    ...
    // Handles if there are black tiles
    let newHint
...
// 2D array of across and down hints
